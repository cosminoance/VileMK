 ...
 *  shows beside the heading while it is closed. `accent` marks a fold the user
 *  should not miss: accent border and title, and a blinking cursor while closed. */
export function Fold({ title, open, onToggle, summary, accent, children }:
    { title: string; open: boolean; onToggle: (open: boolean) => void;
      summary?: ReactNode; accent?: boolean; children: ReactNode }) {
  const id = useId();
